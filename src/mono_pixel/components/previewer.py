"""Preview generation helpers for terminal output."""
...
def generate_ascii_preview(image: Image.Image, preview_width: int = 80) -> str:
    ...
    preview_height = max(1, int(preview_width * aspect_ratio * 0.5))
...
def build_position_aware_preview(
    image: Image.Image, image_size: tuple[int, int], preview_width: int = 80
) -> str:
    original_width, original_height = image.size
...
class Previewer:
    """Generate terminal-friendly previews."""

    def generate_ascii_preview(
        self, image: Image.Image, preview_width: int = 80
    ) -> str:
        original_width, original_height = image.size
        aspect_ratio = original_height / original_width
        preview_height = max(1, int(preview_width * aspect_ratio * 0.5))
    ...
    def build_position_aware_preview(
        self, image: Image.Image, image_size: tuple[int, int], preview_width: int = 80
    ) -> str:
        original_width, original_height = image.size
        ascii_preview = self.generate_ascii_preview(image, preview_width)
        rows = ascii_preview.splitlines()
        ...
        preview_height = len(rows)
        grid = [list(row) for row in rows]
        content_bbox = self.detect_content_bbox(image)
        bbox_summary = "none"
